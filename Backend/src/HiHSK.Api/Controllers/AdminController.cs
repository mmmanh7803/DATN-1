...
public class AdminController : ControllerBase
{
    ...
    /// <summary>
    /// Kiểm tra các bảng nào chưa có dữ liệu
    /// </summary>
    [HttpGet("check-empty-tables")]
    [AllowAnonymous]
    public async Task<IActionResult> CheckEmptyTables()
    {
        try
        {
            var emptyTables = new List<object>();

            // Danh sách tất cả các bảng cần kiểm tra (trừ Identity tables)
            var tableChecks = new Dictionary<string, Task<int>>
            {
                { "CourseCategories", _context.CourseCategories.CountAsync() },
                { "Courses", _context.Courses.CountAsync() },
                { "Lessons", _context.Lessons.CountAsync() },
                { "LessonTopics", _context.LessonTopics.CountAsync() },
                { "LessonExercises", _context.LessonExercises.CountAsync() },
                { "Words", _context.Words.CountAsync() },
                { "VocabularyTopics", _context.VocabularyTopics.CountAsync() },
                { "WordVocabularyTopics", _context.WordVocabularyTopics.CountAsync() },
                { "Radicals", _context.Radicals.CountAsync() },
                { "WordRadicals", _context.WordRadicals.CountAsync() },
                { "Dialogues", _context.Dialogues.CountAsync() },
                { "DialogueSentences", _context.DialogueSentences.CountAsync() },
                { "ReadingPassages", _context.ReadingPassages.CountAsync() },
                { "ReadingPassageWords", _context.ReadingPassageWords.CountAsync() },
                { "Questions", _context.Questions.CountAsync() },
                { "QuestionOptions", _context.QuestionOptions.CountAsync() },
                { "ExamPapers", _context.ExamPapers.CountAsync() },
                { "ExamPaperQuestions", _context.ExamPaperQuestions.CountAsync() },
                { "SentencePatterns", _context.SentencePatterns.CountAsync() },
                { "SentencePatternExamples", _context.SentencePatternExamples.CountAsync() },
                { "MeasureWords", _context.MeasureWords.CountAsync() },
                { "MeasureWordExamples", _context.MeasureWordExamples.CountAsync() },
                { "WordMeasureWords", _context.WordMeasureWords.CountAsync() },
                { "WordExamples", _context.WordExamples.CountAsync() },
                { "WritingExercises", _context.WritingExercises.CountAsync() },
                { "TranslationHistories", _context.TranslationHistories.CountAsync() },
                { "UserLessonProgresses", _context.UserLessonProgresses.CountAsync() },
                { "UserAnswers", _context.UserAnswers.CountAsync() },
                { "UserWordProgresses", _context.UserWordProgresses.CountAsync() },
                { "UserLessonStatuses", _context.UserLessonStatuses.CountAsync() },
                { "UserCourseStatuses", _context.UserCourseStatuses.CountAsync() },
                { "UserDialogueProgresses", _context.UserDialogueProgresses.CountAsync() },
                { "UserActivityProgresses", _context.UserActivityProgresses.CountAsync() },
                { "UserReadingProgresses", _context.UserReadingProgresses.CountAsync() },
                { "UserReadingWordMarks", _context.UserReadingWordMarks.CountAsync() },
                { "UserRadicalProgresses", _context.UserRadicalProgresses.CountAsync() },
                { "UserWritingProgresses", _context.UserWritingProgresses.CountAsync() },
                { "UserWritingAttempts", _context.UserWritingAttempts.CountAsync() },
                { "UserPronunciationAttempts", _context.UserPronunciationAttempts.CountAsync() },
                { "FavoriteWords", _context.FavoriteWords.CountAsync() },
                { "FavoriteSentencePatterns", _context.FavoriteSentencePatterns.CountAsync() },
                { "ExamResultAnalyses", _context.ExamResultAnalyses.CountAsync() },
                { "Leaderboards", _context.Leaderboards.CountAsync() },
                { "UserDailyStats", _context.UserDailyStats.CountAsync() },
            };

            // Thực hiện tất cả các kiểm tra song song và lấy kết quả
            foreach (var tableCheck in tableChecks)
            {
                var count = await tableCheck.Value;
                if (count == 0)
                {
                    emptyTables.Add(new { tableName = tableCheck.Key, count = count });
                }
            }

            return Ok(new
            {
                emptyTablesCount = emptyTables.Count,
                emptyTables = emptyTables,
                message = $"Tìm thấy {emptyTables.Count} bảng chưa có dữ liệu"
            });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Lỗi khi kiểm tra bảng trống");
            return StatusCode(500, new { message = "Lỗi khi kiểm tra bảng trống", error = ex.Message });
        }
    }
